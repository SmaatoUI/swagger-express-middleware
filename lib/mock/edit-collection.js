...
/**
 * Resolves the schema properties.
 *
 * @param   {object}    schema      The JSON schema for the data.
 * @returns {object}    properties  The first level schema properties.
 */
function resolveSchemaProperties(schema) {
  if (schema.allOf) {
    ...
    schema.allOf.forEach(function(subSchema) {
      Object.assign(properties, resolveSchemaProperties(subSchema));
    });
...
function getResourceNameByName(data, schema) {
  ...
  var schemaProperties = resolveSchemaProperties(schema);

  // Get a list of all existing and possible properties of the resource
  var propNames = _.union(_.keys(schemaProperties), _.keys(data));
  ...
  var foundMatch = nameProperties.some(function(lowercasePropName) {
    ...
    if (index >= 0) {
      ...
      if (schemaProperties[propInfo.name]) {
        propInfo.schema = schemaProperties[propInfo.name];
      }
